...
abstract class RDD[T: ClassTag](
  ...
  /**
    ...
    * 本地checkpointing是用性能换可靠性，local checkpointed data 是短暂写在executors的本地存储。如果
    * executor在计算的过程中失败了，数据将变得不可访问
